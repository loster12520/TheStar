...
|------|------|--------|--------|------|------|--------|
| 单元测试 | `SignalTest.kt` | 23 | — | — | — | — |
| 单元测试 | `MemoTest.kt` | 29 | — | — | — | — |
| 单元测试 | `EffectTest.kt` | 19 | — | — | — | — |
| 单元测试 | `BatchTest.kt` | 12 | — | — | — | — |
| 单元测试 | `UntrackTest.kt` | 11 | — | — | — | — |
| 单元测试 | `DisposableTest.kt` | 13 | — | — | — | — |
| 综合测试 | `IntegrationTest.kt` | 15 | — | — | — | — |
| 综合测试 | `AsyncTest.kt` | 12 | — | — | — | — |
| 压力测试 | `StressTest.kt` | 16 | — | — | — | — |
| **合计** | **9 个文件** | **150** | **** | **** | **** | **** |
...
| 17 | `signal dispose can be called on direct Signal` | Dispose |
| 18 | `signal dispose is idempotent` | Dispose |
| 19 | `signal dispose clears observer links` | Dispose |
| 20 | `signal value is still readable after dispose` | Dispose |
| 21 | `signal value is still writable after dispose` | Dispose |
| 22 | `signal returns Signal instance` | 类型 |
| 23 | `signal is Disposable` | 类型 |
...
| 14 | `effect exception does not prevent other effects` | 异常隔离 |
...
| # | 测试用例 | 分类 |
|---|----------|------|
| 1 | `Signal implements Disposable` | 类型检查 |
| 2 | `Memo implements Disposable` | 类型检查 |
| 3 | `Effect implements Disposable` | 类型检查 |
| 4 | `Signal dispose is idempotent` | Signal dispose |
| 5 | `Signal dispose clears observers` | Signal dispose |
| 6 | `Signal dispose multiple times keeps observers empty` | Signal dispose |
| 7 | `Memo dispose is idempotent` | Memo dispose |
| 8 | `Memo dispose clears sources and observers` | Memo dispose |
| 9 | `Memo dispose removes itself from upstream signal` | Memo dispose |
| 10 | `Effect dispose is idempotent` | Effect dispose |
| 11 | `Effect dispose clears sources from upstream` | Effect dispose |
| 12 | `dispose chain - effect after all disposes do not trigger` | 组合 dispose |
| 13 | `partial dispose - disposing signal does not affect disposed effect` | 组合 dispose |
...
| 3 | `dispose 10000 signals` | 10,000 | 无崩溃 |
| 4 | `deep chain of 100 lazy memos` | 100 深链 | 传播正确性 |
| 5 | `deep chain of 100 eager memos` | 100 深链 | 传播正确性 |
| 6 | `deep chain propagation correctness` | 200 深链 | 传播正确性 |
| 7 | `one signal with 1000 effects` | 1,000 observer | 宽依赖 |
| 8 | `one signal with 1000 memos` | 1,000 派生 | 宽依赖 |
| 9 | `batch with 10000 writes` | 10,000 次写入 | 批处理 |
| 10 | `batch with 10000 signal writes to 100 signals` | 100×100 | 批处理 |
| 11 | `create and dispose 1000 effects on same signal` | 1,000 循环 | 无泄漏 |
| 12 | `create and dispose 1000 memos on same signal` | 1,000 循环 | 无泄漏 |
| 13 | `complex graph with 500 nodes - correctness` | 500 节点 | 正确性 |
| 14 | `wide fan out and deep chain combined` | 10×50 链 | 组合场景 |
| 15 | `dispose all 10000 signals no leak` | 10,000 | 无泄漏 |
| 16 | `large nested batch does not stack overflow` | 500 层嵌套 | 栈安全 |
